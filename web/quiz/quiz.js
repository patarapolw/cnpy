...
let newMeaningExplanationTimestamp = new Date();
...
makeToast.toasted = new Set();

function makeToast({ v, q_user, explanation, correct, attrValue }) {
  ...
  if (makeToast.toasted.has(v)) return;
  const duration = (correct ? 5 : 20) * 1000;

  makeToast.toasted.add(v);
  setTimeout(() => {
    makeToast.toasted.delete(v);
  }, duration);
...
function on_ai_ask(param) {
  ...
  if (meaning) {
    ...
    if (checkSameItem()) {
      if (explanation) {
        elMeaningExplanation.innerHTML = converter.makeHtml(explanation);
      }
      ...
      // If timestamp since new explantion elapsed for less than 5 seconds (5000 milli)
      // LLM evaluation of the last vocab might not been read properly yet
      // show the toast even if correct = true
      if (+new Date() - +newMeaningExplanationTimestamp < 5000) {
        isToast = true;
      }
    }
    ...
    if (isToast) {
      makeToast({ v, q_user, explanation, correct, attrValue });
      return;
    } else {
